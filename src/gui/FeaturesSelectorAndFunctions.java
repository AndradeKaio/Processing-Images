	...
	public static boolean isFeaturesAndAngleReady(){
		if(angle != -1){
			if(selectedFeatures.size() == 2){
				...
				System.out.println("erro: Numero de descritores selecionados diferente de 2");
				return false;
			}
		}else{
			System.out.println("erro: Distancia da Matrix de Coocorrencia nao foi selecionada");
			return false;
		}
	}
...
//				findBestParameters();
		...
		this.getChildren().addAll(addRadioButtom(),addCheckBox(),b,b2,bt);
	}
	...
	private String findBestParameters(){
		double best = 0;
		String res ="";
		ArrayList<ArrayList<Mat>> images = FileManager.readTrainFile(new File("C:/Users/Kaio Henrique/workspace/pid"));
		int i=0;
		double accuracy=0.0;
			...
			for(int k1 = 0; k1<FEATURES.length; k1++){
				for(int k2 = 0; k2<FEATURES.length; k2++){
					if(k1 != k2){
						...
						accuracy =Test.readTestFile(new File("C:/Users/Kaio Henrique/workspace/pid/test.txt"),0)[0];
						if(best < accuracy){
							best = accuracy;
	...
	private VBox addCheckBox(){
		...
		CheckBox contraste = new CheckBox("Contrast");
		...
		CheckBox homogeneidade = new CheckBox("Homogeneity");
