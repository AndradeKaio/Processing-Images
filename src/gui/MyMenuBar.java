	...
	private Menu menuClassification()
	{
		...
		toClassify.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				{
					Timer.initTime();
					Main.messagePane.addNewMessage("Classified image as type "+ Classify.classify_mahalanobis(Main.imagePane2.image));
					Timer.finalTime();
					//Main.messagePane.addNewMessage("Program finish in "+Timer.total()+" ms");
					Main.messagePane.addNewMessage(Timer.result());
...
//					Main.imagePane1.unsetImageSelection();
	...
	private Menu menuFile()
	{
        ...
        menuFile.getItems().addAll(loadImage, editImage);
        ...
        loadImage.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
                System.out.println("Open image - Ctrl+O");
                File file = ImageManager.fileChooser(null);
                Main.imagePane1.loadImage(ImageManager.file2Mat(file));
            }
        });
        
        editImage.setAccelerator(KeyCombination.keyCombination("Ctrl+E"));
